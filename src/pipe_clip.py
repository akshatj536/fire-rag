...
class CLIPEmbeddings(Embeddings):
    ...
    def embed_image(self, image_base64: str) -> List[float]:
        ...
        try:
            ...
            logging.error(f"Failed to embed image: {e}")
            return []

    def embed_multimodal(self, text: str, images: List[str]) -> List[float]:
        """Embed combined text and images"""
        text_emb = np.array(self.embed_query(text))

        if not images:
            return text_emb.tolist()

        image_embs = [np.array(self.embed_image(img)) for img in images if img]
        if not image_embs:
            return text_emb.tolist()
...
class MultimodalRAGPipeline:
    ...
    def create_chunks_by_title(self, elements):
        ...
        logging.info("Creating chunks by title...")
    ...
    def create_ai_summary(self, text: str, tables: List[str], images: List[str]) -> str:
        ...
        prompt_text = f"""You are an expert content summarizer. Analyze the provided text, tables, and images and create a clear, concise, and structured summary.
...
"""
    ...
    def process_chunks(self, chunks) -> List[Document]:
        ...
        logging.info("Processing chunks and creating summaries...")
        langchain_documents = []
        ...
        for i, chunk in enumerate(chunks):
            ...
            if content_data["tables"] or content_data['images']:
                logging.info(f"  - Multimodal content detected: {content_data['types']}")
        ...
        logging.info(f"Processed {len(langchain_documents)} documents")
        return langchain_documents

    def create_vector_store(self, docs: List[Document]):
        """Create ChromaDB vector store with CLIP embeddings"""
        logging.info("Creating vector store with CLIP embeddings...")
        texts, embeddings_list, metadatas = [], [], []
        ...
        self.vectorstore = Chroma.from_texts(
            ...
            metadatas=metadatas,
            persist_directory=settings.CHROMA_CLIP_DB_PATH,
            collection_metadata={"hnsw:space": "cosine"}
        )
        logging.info(f"Vector store created and persisted to {settings.CHROMA_CLIP_DB_PATH}")
        return self.vectorstore

    def generate_answer(self, chunks: List[Document], query: str) -> str:
        """Generate final answer using retrieved chunks"""
        logging.info("Generating answer...")
        prompt_text = f"""You are an expert assistant. Analyze the provided documents (text, tables, and images) and give a clear, accurate, and well-structured answer to the user query.
...
"""
    ...
    def process_pdf(self, file_path: str):
        ...
        logging.info("=" * 60)
        logging.info("STARTING MULTIMODAL RAG PIPELINE WITH CLIP")
        logging.info("=" * 60)
        ...
        logging.info("=" * 60)
        logging.info("PIPELINE COMPLETE - Ready for queries!")
        logging.info("=" * 60)
        return self
